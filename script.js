...
function startQuiz() {
  ...
  // We save the 'path' object which contains uni, sem, course, grade, etc.
  localStorage.setItem('neoQuiz_lastSession', JSON.stringify(state.path));
  // ==========================================
...
window.addEventListener('load', () => {
  const lastSession = localStorage.getItem('neoQuiz_lastSession');
  if (lastSession) {
    const data = JSON.parse(lastSession);
    const container = document.getElementById('system-restore-container');
    const title = document.getElementById('restore-course-name');
    const details = document.getElementById('restore-details');
    
    // Update text
    title.textContent = data.course || ("Grade " + data.grade);
    
    let detailText = "Continue where you left off";
    if(data.type === 'freshman') detailText = `${data.university} • ${data.semester}`;
    if(data.type === 'highschool') detailText = `High School Exam Prep`;
...
// 2. Function called by the Resume Button
function resumeSession() {
  ...
  if (lastSession) {
    ...
    // Load the correct questions based on saved path
    if (data.type === 'freshman') {
      state.currentQuizSet = quizData.freshman[data.semester][data.course];
    } else if (data.type === 'highschool') {
      state.currentQuizSet = quizData.highschool[data.grade];
    }
    
    // Start
    if (state.currentQuizSet) {
      startQuiz();
    } else {
